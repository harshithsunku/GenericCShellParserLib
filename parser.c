...
command_parser(void){
    ...
    while(1){
        ...
        char *temp_input = readline(console_name);
        if (!temp_input) {
            printf("error in reading from stdin\n");
            break;
        }

        strncpy(cons_input_buffer, temp_input, sizeof(cons_input_buffer) - 1); 
        cons_input_buffer[sizeof(cons_input_buffer) - 1] = '\0'; 


        if (strlen(cons_input_buffer) > 0) {
            add_history(cons_input_buffer); // Add non-empty commands to history
